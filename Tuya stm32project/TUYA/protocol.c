...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_SCREEN, DP_TYPE_BOOL},
  {DPID_LIGHT, DP_TYPE_BOOL},
  {DPID_LED_MODE, DP_TYPE_ENUM},
};
...
void uart_transmit_output(unsigned char value)
{
    //#error "请将MCU串口发送函数填入该函数,并删除该行"
		//解决第一个数据丢失
	while (USART_GetFlagStatus(USART2, USART_FLAG_TXE) == RESET); //传输完成标志位
...
void all_data_update(void)
{
    ...
    mcu_dp_bool_update(DPID_SWITCH,当前灯联动开关); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_SCREEN,当前屏保开关); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_LIGHT,当前自动亮度); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_LED_MODE,当前灯模式); //枚举型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_screen_handle
功能描述 : 针对DPID_SCREEN的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_screen_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    screen = mcu_get_dp_download_bool(value,length);
    if(screen == 0) {
			All.Lock_State = 1;//设为开
			Lock_config();//由开转灭
        //开关关
    }else {
			All.Lock_State = 0;//设为灭
			Lock_config(); //由灭转开   
			//开关开
    }
  
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_SCREEN,screen);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_led_mode_handle
功能描述 : 针对DPID_LED_MODE的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_led_mode_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char led_mode;
    
    led_mode = mcu_get_dp_download_enum(value,length);
    switch(led_mode) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_LED_MODE, led_mode);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_SCREEN:
            //屏保开关处理函数
            ret = dp_download_screen_handle(value,length);
        break;
        case DPID_LIGHT:
            //自动亮度处理函数
        ...
        case DPID_LED_MODE:
            //灯模式处理函数
            ret = dp_download_led_mode_handle(value,length);
        break;
   ...
   */
    if(time[0] == 1) {
        //正确接收到wifi模块返回的本地时钟数据
			//USART2_printf("[time] %d/%d/%d %d:%d:%d",time[1],time[2],time[3],time[4],time[5],time[6]);
			RTC_Set(time[1]+2000,time[2],time[3],time[4]+8,time[5],time[6]);//设置RTC时间
		}else {
...
void weather_data_user_handle(char *name, unsigned char type, const unsigned char *data, char day)
{
    ...
    if(my_strcmp(name, "temp") == 0) {
				Data_set(wea_tem,value_int);//温度
    }else if(my_strcmp(name, "humidity") == 0) {
				Data_set(wea_hum,value_int);//湿度
		}else if(my_strcmp(name, "pm25") == 0) {
				Data_set(wea_pm25,value_int);//pm25
